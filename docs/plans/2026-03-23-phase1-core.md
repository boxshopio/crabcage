...
**Architecture:** Node.js/TypeScript CLI that reads `.crabcage.yml`, validates credentials on the host, generates a `docker-compose.yml`, and launches a hardened Debian-slim container with Claude Code. The CLI orchestrates Docker — it never runs inside the container.
...
│   ├── config/
│   │   ├── schema.ts              # TypeScript types + JSON schema for .crabcage.yml
...
- [x] `.crabcage.yml` config file support with JSON schema validation
